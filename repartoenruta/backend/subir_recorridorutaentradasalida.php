...
for ($sqiv = 0; $sqiv < 7; $sqiv++) {
    ...
    $stmtq = $dbh->prepare('SELECT * FROM recorridorutaentradasalida WHERE idhw = ? ');
//        var_dump($stmtq);
...
if (true) {
    ...
    for ($sqi = 0; $sqi < 1000; $sqi++) {
        ...
        $stmtq = $dbh->prepare('SELECT * FROM recorridorutaentradasalida WHERE id = ?  LIMIT 1');
//        var_dump($stmtq);
//        var_dump($stmtq->execute(array($id)));
//        var_dump($stmtq);
...
//    $sqvqeiedrr[] = $sqvbjo;
    ...
    if (isset($objsqrowreq->kilometrajetablero)) {

    } else {
        header("HTTP/1.0 202 ErrorqServerq");
        echo '["el dato kilometrajetablero -row es obligatorio"]';
        die();
    }

    $infociloq = trim($objsqrowreq->kilometrajetablero);

    $sqvbjo = new \stdClass();
    $sqvbjo->nameciloq = 'kilometrajetablero';
    $sqvbjo->infociloq = $infociloq;
    $sqvqeiedrr[] = $sqvbjo;
    ...
    if (isset($objsqrowreq->gasolinanivel)) {

    } else {
        header("HTTP/1.0 202 ErrorqServerq");
        echo '["el dato gasolinanivel -row es obligatorio"]';
        die();
    }

    $infociloq = trim($objsqrowreq->gasolinanivel);

    $sqvbjo = new \stdClass();
    $sqvbjo->nameciloq = 'gasolinanivel';
    $sqvbjo->infociloq = $infociloq;
    $sqvqeiedrr[] = $sqvbjo;
    ...
    if (isset($objsqrowreq->tiporegistrotipe)) {

    } else {
        header("HTTP/1.0 202 ErrorqServerq");
        echo '["el dato tiporegistrotipe -row es obligatorio"]';
        die();
    }

    $infociloq = trim($objsqrowreq->tiporegistrotipe);

    $sqvbjo = new \stdClass();
    $sqvbjo->nameciloq = 'tiporegistrotipe';
    $sqvbjo->infociloq = $infociloq;
    $sqvqeiedrr[] = $sqvbjo;
    ...
    if (isset($objsqrowreq->termina_id_recorridorutaentradasalida)) {

    } else {
        header("HTTP/1.0 202 ErrorqServerq");
        echo '["el dato termina_id_recorridorutaentradasalida -row es obligatorio"]';
        die();
    }

    $infociloq = trim($objsqrowreq->termina_id_recorridorutaentradasalida);

    $sqvbjo = new \stdClass();
    $sqvbjo->nameciloq = 'termina_id_recorridorutaentradasalida';
    $sqvbjo->infociloq = $infociloq;
    $sqvqeiedrr[] = $sqvbjo;
...
//$sqvrsqdvbsdqsvvs = substr($sqvrsqdvbsdqsvvs, 0, strlen($sqvrsqdvbsdqsvvs) - 1);
        ...
        $stmt = $dbh->prepare("INSERT INTO recorridorutaentradasalida (" . $sqvrsqdvbsdqsvvs . ") VALUES (" . $sqvrsqdpdpqpsp . ")");
...
//            . "  WHERE id=?" );
//
//        $stmt = $dbh->prepare("UPDATE  recorridorutaentradasalida SET " . $sqvrsqdvbsdqsvvs
//            . "  WHERE idhw=?" );
...
if ('agregar' == $sqvr_req_act_doqaction) {
    ...
    //$sqvrsqrsqexec_rr[] = "UPDATE recorridorutaentradasalida SET subido=1 WHERE id='".$objsqrowreq->id."'";

    if ($sqvr_existerowrequestbandera) {
        $sqvrsqrsqexec_rr[] = "UPDATE recorridorutaentradasalida SET subido=1 WHERE id='" . $objsqrowreq->id . "'";
        $sqvrsqrsqexec_rr[] = "UPDATE recorridorutaentradasalida SET subidomodificado=(subidomodificado - 1) WHERE id='" . $objsqrowreq->id . "'";
    } else {
        $sqvrsqrsqexec_rr[] = "UPDATE recorridorutaentradasalida SET subido=1 WHERE id='" . $objsqrowreq->id . "'";
    }
    ...
    //$sqvrsqrsqexec_rr[] = "UPDATE recorridorutaentradasalida SET subidomodificado=1 WHERE id='".$objsqrowreq->id."'";
