...
class SSP {
	...
	{
		$out = array();
$sqvrorigcolumns=$columns;

        for ($sgbqwi=0;$sgbqwi < count($columns);$sgbqwi++){
            $sqdghjj='';
            $sqfvvcol=array();
            foreach ($columns[$sgbqwi] as $clave => $valor) {
                if('db'==$clave){
                    //$sqdghjjwdd=$valor;
                    $sqdghjjwdd=explode(' as ', $valor);
                    $sqfvvcol[$clave] =$sqdghjjwdd[1];
                }
                else {
                    $sqfvvcol[$clave] =$valor;
                    }
            }

            $sqsffc[$sgbqwi]=$sqfvvcol;
        }
$columns=$sqsffc;
	...
	static function order ( $request, $columns )
	{
		...
		if ( isset($request['order']) && count($request['order']) ) {
			...
			for ( $i=0, $ien=count($request['order']) ; $i<$ien ; $i++ ) {
               ...
               // 'db' => 'visitatienda.fechahora_hwa as visitatienda_qasq_fechahora_hwa', 'dt' => 'visitatienda_qasq_fechahora_hwa'
...
//        }
        ...
        $order = self::order( $request, $sqsffc );
