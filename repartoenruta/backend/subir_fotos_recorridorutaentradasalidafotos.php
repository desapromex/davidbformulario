...
for ($sqi = 0; $sqi < 7; $sqi++) {
    ...
    //$stmtq = $dbh->prepare('SELECT * FROM recorridorutaentradasalidafotos WHERE id = ?  LIMIT 1');
    $stmtq = $dbh->prepare('SELECT * FROM recorridorutaentradasalidafotos WHERE idhw = ?  ');
//        var_dump($stmtq);
...
if (true) {
    ...
    for ($sqi = 0; $sqi < 1000; $sqi++) {
        ...
        $stmtq = $dbh->prepare('SELECT * FROM recorridorutaentradasalidafotos WHERE id = ?  LIMIT 3');
//        var_dump($stmtq);
//        var_dump($stmtq->execute(array($id)));
//        var_dump($stmtq);
...
//
//    "
//CREATE TABLE `recorridorutaentradasalidafotos` (
//bajado INT NOT NULL,
//subido INT NOT NULL,
...
//    $sqvqeiedrr[] = $sqvbjo;
    ...
    if (isset($objsqrowreq->id_recorridorutaentradasalida)) {

    } else {
        header("HTTP/1.0 202 ErrorqServerq");
        echo '["el dato id_recorridorutaentradasalida -row es obligatorio"]';
        die();
    }

    $infociloq = trim($objsqrowreq->id_recorridorutaentradasalida);

    $sqvbjo = new \stdClass();
    $sqvbjo->nameciloq = 'id_recorridorutaentradasalida';
    $sqvbjo->infociloq = $infociloq;
    $sqvqeiedrr[] = $sqvbjo;
...
//    https://riandamdb7.buysaleclub.com/rianda_md_apiv1b/uploads/2022/09/18/
                ...
                $sqvrfoldqeruploads = 'uploads_folder_web';
                $sqvr_sdcvvruta = $sqvr_anio . '/' . $sqvr_mes . '/' . $sqvr_dia . '/' . $sqvr_hora . '/' . $sqvr_minuto . '/' . $sqvr_segundo;
...
//$sqvrsqdvbsdqsvvs = substr($sqvrsqdvbsdqsvvs, 0, strlen($sqvrsqdvbsdqsvvs) - 1);
        ...
        $stmt = $dbh->prepare("INSERT INTO recorridorutaentradasalidafotos (" . $sqvrsqdvbsdqsvvs . ") VALUES (" . $sqvrsqdpdpqpsp . ")");
...
//            . "  WHERE id=?" );
//
//        $stmt = $dbh->prepare("UPDATE  recorridorutaentradasalidafotos SET " . $sqvrsqdvbsdqsvvs
//            . "  WHERE idhw=?" );
...
//
//    if($sqvr_existerowrequestbandera) {
//        $sqvrsqrsqexec_rr[] = "UPDATE recorridorutaentradasalidafotos SET subido=1 WHERE id='" . $objsqrowreq->id . "'";
//        $sqvrsqrsqexec_rr[] = "UPDATE recorridorutaentradasalidafotos SET subidomodificado=1 WHERE id='" . $objsqrowreq->id . "'";
//    }
//    else{
//        $sqvrsqrsqexec_rr[] = "UPDATE recorridorutaentradasalidafotos SET subido=1 WHERE id='" . $objsqrowreq->id . "'";
//    }
...
if('agregar'==$sqvr_req_act_doqaction){
    ...
    if($sqvr_existerowrequestbandera) {
        $sqvrsqrsqexec_rr[] = "UPDATE recorridorutaentradasalidafotos SET subido=1 WHERE id='" . $objsqrowreq->id . "'";
        $sqvrsqrsqexec_rr[] = "UPDATE recorridorutaentradasalidafotos SET subidomodificado=(subidomodificado - 1) WHERE id='" . $objsqrowreq->id . "'";
    }
    else{
        $sqvrsqrsqexec_rr[] = "UPDATE recorridorutaentradasalidafotos SET subido=1 WHERE id='" . $objsqrowreq->id . "'";
    }
